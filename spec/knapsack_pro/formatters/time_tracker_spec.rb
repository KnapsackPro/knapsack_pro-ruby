...
describe "TimeTracker" do
  ...
  describe "#queue" do
    ...
    it "rspec split by test example" do
      ...
      run_specs(spec, "queue", env: "TEST__SBTE=1") do |spec_paths, queue|
        ...
        expect(queue.find { |time| time["path"] == "#{spec_path}[1:1]" }["time_execution"]).to be_between(0.00, 0.05)
  ...
  describe "#duration" do
    it do
      ...
      run_specs(spec, "duration") do |_, duration|
        expect(duration).to be_between(0.00, 0.05)
      end
    end
  end
  ...
  describe "#scheduled_paths" do
    it "returns the most recent scheduled_paths" do
      spec0 = <<~SPEC
        RSpec.configure do |config|
          config.before(:all) do
            time_tracker = ::RSpec.configuration.formatters.find { |f| f.class.to_s == "TestableTimeTracker" }
      ...
      run_specs([spec0, spec1], "scheduled_paths") do |spec_paths, scheduled_paths|
        expect(scheduled_paths.size).to eq(1)
        expect(scheduled_paths[0]).to eq(spec_paths[1])
      end
    end
  end
