...
module KnapsackPro
  class QueueAllocator
    ...
    def test_file_paths(can_initialize_queue, executed_test_files, time_tracker: nil)
      ...
      batch = pull_tests_from_queue(can_initialize_queue, time_tracker: time_tracker)
      ...
      return try_initializing_queue(test_files_result.test_files, time_tracker: time_tracker) if test_files_result.quick?
      ...
      try_initializing_queue(test_files_result.test_files, time_tracker: time_tracker)
    end
    ...
    def build_action_v1(can_initialize_queue:, attempt_connect_to_queue:, test_files: nil)
    ...
    def build_action_v2(can_initialize_queue:, attempt_connect_to_queue:, time_tracker:, test_files: nil)
      if can_initialize_queue && !attempt_connect_to_queue
        raise 'Test files are required when initializing a new queue.' if test_files.nil?
        test_files = KnapsackPro::Crypto::Encryptor.call(test_files)
      end
      ...
      KnapsackPro::Client::API::V2::Queues.queue(
        ...
        node_build_id: ci_node_build_id,
        test_files: test_files,
        test_paths: time_tracker.scheduled_paths,
        failed_test_id_paths: time_tracker.failed_test_id_paths
      )
    end

    def pull_tests_from_queue(can_initialize_queue, time_tracker: nil)
      if time_tracker.nil?
        pull_tests_from_queue_v1(can_initialize_queue)
      else
        pull_tests_from_queue_v2(can_initialize_queue, time_tracker)
      end
    end

    def pull_tests_from_queue_v1(can_initialize_queue)
      action = build_action_v1(can_initialize_queue: can_initialize_queue, attempt_connect_to_queue: can_initialize_queue)
    ...
    def initialize_queue_v1(tests_to_run)
      action = build_action_v1(can_initialize_queue: true, attempt_connect_to_queue: false, test_files: tests_to_run)
      connection = KnapsackPro::Client::Connection.new(action)
      response = connection.call
      Batch.new(connection, response)
    end

    def initialize_queue_v2(tests_to_run, time_tracker)
      action = build_action_v2(can_initialize_queue: true, attempt_connect_to_queue: false, test_files: tests_to_run, time_tracker: time_tracker)
      connection = KnapsackPro::Client::Connection.new(action)
      response = connection.call
      Batch.new(connection, response)
    end

    def try_initializing_queue(tests, time_tracker: nil)
      if time_tracker.nil?
        try_initializing_queue_v1(tests)
      else
        try_initializing_queue_v2(tests, time_tracker)
      end
    end

    def try_initializing_queue_v1(tests)
      result = initialize_queue_v1(tests)
      return switch_to_fallback_mode(executed_test_files: []) if result.connection_failed?

      normalize_test_files(result.test_files)
    end

    def try_initializing_queue_v2(tests, time_tracker)
      result = initialize_queue_v2(tests, time_tracker)
      return switch_to_fallback_mode(executed_test_files: []) if result.connection_failed?
