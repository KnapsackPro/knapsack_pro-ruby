...
module KnapsackPro
  class QueueAllocator
    ...
    def test_file_paths(can_initialize_queue, executed_test_files, time_tracker: nil, batch_uuid: SecureRandom.uuid)
    ...
    def pull_tests_from_queue_v2(can_initialize_queue, batch_uuid, time_tracker)
      action = build_action_v2(can_initialize_queue: can_initialize_queue, attempt_connect_to_queue: can_initialize_queue, time_tracker: time_tracker, batch_uuid: batch_uuid)
      connection = KnapsackPro::Client::Connection.new(action)
      response = connection.call
      Batch.new(connection, response)
    end
    ...
    def initialize_queue_v2(tests_to_run, batch_uuid, time_tracker)
      action = build_action_v2(can_initialize_queue: true, attempt_connect_to_queue: false, test_files: tests_to_run, time_tracker: time_tracker, batch_uuid: batch_uuid)
      connection = KnapsackPro::Client::Connection.new(action)
      response = connection.call
      Batch.new(connection, response)
    end
